#!/usr/bin/env bash
start_dir=`pwd`
setting_repo="Tbabm/vim_settings"
curl -fSsL https://raw.githubusercontent.com/egalpin/apt-vim/master/install.sh|sh

if [ -e ~/.vimrc ]; then
    mv ~/.vimrc ~/.vimrc.bak
fi
curl -fSsLo vimrc https://raw.githubusercontent.com/${setting_repo}/master/vimrc
cp vimrc ~/.vimrc
...
curl -fSsLo vim_config.json https://raw.githubusercontent.com/${setting_repo}/master/vim_config.json
